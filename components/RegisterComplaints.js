...
function RegisterComplaints() {
	...
	const postComplaint = async () => {
		try {
			...
			const msg = response.data.rowCount > 0 ? `Complaint with ID ${id} registered sucessfully` : "Complaint not registered sucessfully";
	...
	return (
		<ScrollView style={[styles.container, { backgroundColor: bgColor }]}>
			<TextInput maxLength={50} placeholder="Enter Complaint Title" onChangeText={handleTextChange} value={text} style={styles.textInput} />
			<TextInput
				multiline={true}
