...
export default function UserProfile({ route }) {
	...
	useEffect(() => {
		getUserRole() == "Mess Supervisor" ? getMessFee() : "";
	}, []);
	const getMessFee = async () => {
		...
		if (response.data.length == 0) {
			alert(`Mess Fee of Month ${month} is not Available`);
			setUri({ ...uri, messfee: 0 });
		} else setUri({ ...uri, messfee: response.data[0].messfee || 0 });
	};
			...
			<ScrollView contentContainerStyle={{ flex: 1 }}>
				<Card style={stylesn.card}>
					<View style={stylesn.imageSec}>
						<Image source={{ uri: uri.image }} style={stylesn.img} />
					</View>
					<Text style={[stylesn.header]}>{uri.sname.toUpperCase()}</Text>
								...
								<View style={stylesn.row}>
									<Text
										style={{
											...
											fontWeight: "bold",
											display:
												getUserRole() == "Mess Supervisor" && l == "Hostel Fee Status"
													? "none"
													: getUserRole() == "Hostel Supervisor" && l == "Mess Fee Status"
													? "none"
													: "flex",
										}}
									>
					...
					<Button onPress={() => getMessFee()}>Search</Button>
				</Card>
			</ScrollView>
		);
	}
