...
const LoginOrSignUp = (props) => {
  ...
  const Login = async () => {
    const endPoint =
      props.userData[0] == "students" ? "studLogin" : props.userData[0];
    await instance
      ...
      .then(function (response) {
        if (response.data.length == 0) alert("Please Register....");
          ...
          const userN =
            props.userData[0] == "students"
              ? response.data[0].rollno
              : response.data[0].cnic;
      ...
      <View style={[loginStyles.loginContainer, { backgroundColor: bgColor }]}>
        ...
        <ScrollView>
          <View style={loginStyles.textinputs}>
            <Text
              style={[loginStyles.loginSignUpText, { fontFamily: font_Family }]}
            >
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              >
                <Text
                  style={[loginStyles.buttonText, { fontFamily: font_Family }]}
                >
                  SignUp
                </Text>
              </TouchableOpacity>
